...
#if !defined(AI_GESTURE_CNN_INPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_in_1[AI_GESTURE_CNN_IN_1_SIZE_BYTES];
ai_i8* data_ins[AI_GESTURE_CNN_IN_NUM] = {
data_in_1
};
#else
ai_i8* data_ins[AI_GESTURE_CNN_IN_NUM] = {
...
#if !defined(AI_GESTURE_CNN_OUTPUTS_IN_ACTIVATIONS)
AI_ALIGNED(4) ai_i8 data_out_1[AI_GESTURE_CNN_OUT_1_SIZE_BYTES];
ai_i8* data_outs[AI_GESTURE_CNN_OUT_NUM] = {
data_out_1
};
#else
ai_i8* data_outs[AI_GESTURE_CNN_OUT_NUM] = {
...
static ai_handle gesture_cnn = AI_HANDLE_NULL;
...
static int ai_boostrap(ai_handle *act_addr)
{
  ...
  err = ai_gesture_cnn_create_and_init(&gesture_cnn, act_addr, NULL);
  if (err.type != AI_ERROR_NONE) {
    ai_log_err(err, "ai_gesture_cnn_create_and_init");
    return -1;
  }

  ai_input = ai_gesture_cnn_inputs_get(gesture_cnn, NULL);
  ai_output = ai_gesture_cnn_outputs_get(gesture_cnn, NULL);

#if defined(AI_GESTURE_CNN_INPUTS_IN_ACTIVATIONS)
  /*  In the case where "--allocate-inputs" option is used, memory buffer can be
   *  used from the activations buffer. This is not mandatory.
   */
  for (int idx=0; idx < AI_GESTURE_CNN_IN_NUM; idx++) {
   ...
   */
  for (int idx=0; idx < AI_GESTURE_CNN_OUT_NUM; idx++) {
...
static int ai_run(void)
{
  ...
  batch = ai_gesture_cnn_run(gesture_cnn, ai_input, ai_output);
  if (batch != 1) {
    ai_log_err(ai_gesture_cnn_get_error(gesture_cnn),
        "ai_gesture_cnn_run");
    return -1;
  }
