...
f = np.load(os.path.join(BASE_DIR, "camera_data", "train_data.npz"))
...
#保存/加载模型
model.save(os.path.join(BASE_DIR, 'gesture.h5'))

#------------------------------【评估】---------------------------------

f = np.load(os.path.join(BASE_DIR, "camera_data", "test_data.npz"))
...
model = tf.keras.models.load_model(os.path.join(BASE_DIR, 'gesture.h5'))
model.summary()
