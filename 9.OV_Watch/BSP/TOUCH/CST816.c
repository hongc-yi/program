...
/* Apply calibration after display rotation so positive means down on screen. */
#define TOUCH_Y_CALIBRATION 15
#define REVERSE 1
...
void CST816_Get_XY_AXIS(void)
{
	...
	raw_x=((DAT[0]&0x0F)<<8)|DAT[1];
	raw_y=((DAT[2]&0x0F)<<8)|DAT[3];

	#if REVERSE
		CST816_Instance.X_Pos = raw_x < 240U ? 239U - raw_x : 0U;
		CST816_Instance.Y_Pos = raw_y < 280U ? 279U - raw_y : 0U;
	#else
		CST816_Instance.X_Pos = raw_x;
		CST816_Instance.Y_Pos = raw_y;
	#endif

	CST816_Instance.Y_Pos += TOUCH_Y_CALIBRATION;
	if(CST816_Instance.X_Pos > 239U) CST816_Instance.X_Pos = 239U;
	if(CST816_Instance.Y_Pos > 279U) CST816_Instance.Y_Pos = 279U;
}
